...
/**
 * Get all available Groq API keys from environment variables
 */
function getAllApiKeys(): string[] {
  ...
  // Check for multiple keys (GROQ_API_KEY_1, GROQ_API_KEY_2, etc.)
  let index = 1;
  ...
  if (process.env.GROQ_API_KEYS) {
    const commaSeparatedKeys = process.env.GROQ_API_KEYS.split(',').map(k => k.trim()).filter(k => k);
...
if (ALL_API_KEYS.length === 0) {
  ...
  console.log(`✅ Loaded ${ALL_API_KEYS.length} Groq API key(s)`);
...
export async function startInterviewConversation(
  ...
  for (let keyAttempt = 0; keyAttempt < maxKeyAttempts; keyAttempt++) {
    try {
      ...
      console.log(`[Groq] Trying API key ${keyNumber}/${ALL_API_KEYS.length} (${apiKey.substring(0, 10)}...)`);
      ...
      console.log(`[Groq] ✅ Successfully used key ${keyNumber}`);
      ...
      console.log(`[Groq] Key ${keyNumber} failed:`, errorMsg.substring(0, 150));
      ...
      if (errorMsg.includes('quota') || errorMsg.includes('limit') || errorMsg.includes('rate limit') || 
        ...
        console.log(`[Groq] ⚠️  Key ${keyNumber} quota/key error, rotating to next key...`);
        lastKeyError = error;
        continue;
      }
  ...
  // All keys exhausted
  throw new Error(`All Groq API keys exhausted. Last error: ${lastKeyError?.message || 'Unknown error'}. Please check your API keys or wait for quota reset.`);
}
...
export async function continueInterviewConversation(
  ...
  for (let keyAttempt = 0; keyAttempt < maxKeyAttempts; keyAttempt++) {
    try {
      ...
      console.log(`[Groq] ✅ Key ${keyNumber} used for continue conversation`);
      ...
      if (errorMsg.includes('quota') || errorMsg.includes('limit') || errorMsg.includes('rate limit') || 
          errorStatus === 429 || errorStatus === 401 || (errorStatus === 403 && errorMsg.includes('api'))) {
        console.log(`[Groq] ⚠️  Key ${keyNumber} failed, rotating to next key...`);
        lastKeyError = error;
        continue;
      }
